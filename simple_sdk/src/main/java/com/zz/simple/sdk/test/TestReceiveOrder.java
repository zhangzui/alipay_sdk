package com.zz.simple.sdk.test;

import com.zz.simple.sdk.handle.DefaultResquestHandle;
import com.zz.simple.sdk.request.ReceiveOrderRequest;
import org.junit.Test;

/**
 * @author zhangzuizui
 * @date 2018/7/11 12:08
 */
public class TestReceiveOrder {

    @Test
    public void test001(){
        String tradeType = "01";
        String appId = "00001";


        String merPublicKey = "";
        String merPrivateKey = "";

        String JDPublicKey = "";
        String JDPrivateKey = "";

        String chartset = "utf-8";
        String encryptKey = "012345678901234567890123";
        String merchantNo = "00001";
        String version = "1.0.0";

        //构造处理器
        DefaultResquestHandle defaultResquestHandle = new DefaultResquestHandle(
                version,tradeType,appId,merchantNo,chartset,
                merPrivateKey,JDPublicKey,encryptKey);

        //构造请求参数
        ReceiveOrderRequest receiveOrderRequest = new ReceiveOrderRequest();
        receiveOrderRequest.setOrderId("12021231212");

        //执行
        defaultResquestHandle.execute(receiveOrderRequest);
    }
}
